...
Now that you've set up your node and learned how to form a simple transaction, let's get to know the wallet more deeply. The following document is an extremely comprehensive user guide to the reference mugle wallet implementation, `mugle-wallet`.
...
By default, mugle will create all wallet files in the hidden directory `.mugle` under your home directory (i.e. `~/.mugle`). You can also create and use a wallet with data files in a custom directory, as will be explained later.

A mugle wallet maintains its state in an Lightning Memory-Mapped Database (LMDB), with the master seed stored in a separate file. When creating a new wallet, the file structure should be as follows:
...
* `mugle-wallet.toml` contains configuration information for the wallet. You can modify values within to change ports, the address of your mugle node, or logging values.
...
??? note "Tor Configuration &#8628;"
    ...
    `mugle-wallet.toml` contains a [tor] section used to configure values when sending or listening via TOR:
...
`mugle-wallet help` will display all the commands and every global flag.
</br> To get additional info about a specific command type `mugle-wallet help [command]`, e.g:

```text
mugle-wallet help send
```
...
By default, your wallet files will be placed into `~/.mugle`. Alternatively, if you'd like to run a wallet in a directory of your choice, you can create one in the current directory by using flag `-h`, e.g:

```text
mugle-wallet init -h
```

This will create all the needed data files, including `mugle-wallet.toml` and `wallet.seed`, in the current directory. When running any `mugle-wallet` command, mugle will check the working directory if these files exist. If not, it will use the default location `~/.mugle`.
...
```text
mugle-wallet init -hr
```

```text
File /home/tomriddle/wallet/mugle-wallet.toml configured and created
...
```text
mugle-wallet account -c jedusor
```

This will create a new account called 'jedusor'.

All `mugle-wallet` commands can then be passed the argument `-a` to specify an account for the command (otherwise `default` account is used), e.g:

```text
mugle-wallet -a jedusor info
```
...
```text
mugle-wallet address
```

```text
mugle1chv9p4uxp3qgl6qp4w6x5p2434varqwl6fdevg6342qr
```
...
This will automatically configure a Tor hidden service and makes the wallet listen to it for incoming transactions. This allows you to transact directly through Tor with other users who are sending mugles to your `mugle1...`. Your wallet will listen for requests until the process is cancelled (`<Ctrl-C>`).
...
```text
mugle-wallet send -d mugle1dhvv9mvarqwl6fderuxp3qgl6qpphvc9p4u24347ec0mvvg6342q4w6x5r 180
```

This command tries to send `180` mugles to the specified address via Tor. If both wallets are accessible, the transaction would complete immediately with no further steps required.
...
!!! info "Files"
    When sending or receiving via slatepacks, the wallet will also create a text file containing the message it generated. Default path is `~/.mugle/main/slatepacks`.

    To receive mugles using a file, type:

    ```text
    mugle-wallet receive -i $PATH/file.tx
    ```
...
Manually post a finalized transaction to the network. Either type `mugle-wallet post` and enter the slatepack message into the prompt, or specify the file path using the `-i` flag.

```text
mugle-wallet post -i "~/.mugle/main/slatepacks/my_tx.S3.slatepack/"
```

## proof

Mugle's privacy and scalability mechanics mean users no longer have the ability to simply prove a transaction has happened by pointing to it on the chain. By default, whenever a transaction sent to a destination address using `-d`, a payment proof is created.
...
```text
mugle-wallet invoice -d mugle1dhvv9mvarqwl6fderuxp3qgl6qpphvc9p4u24347ec0mvvg6342q4w6x5r 60
```

This command will create an encrypted (since `-d` is provided) invoice, requesting a payment of `60` mugles. The resulting slatepack can then be sent to the other party for them to [pay](#pay).
...
* The wallet will IMMEDIATELY attempt to send the resulting transaction to the wallet listening at: 'mugle1ln4y82fw4urggk3hq0xkeqfhw3dfe6rhcv6a0v64uz4ny9epcc6qpuwx4k'.
...
!!! text ""
    When initializing a wallet from an existing seed via the mugle-wallet `init -r command`, this scan is performed automatically on the first run.
...
```text
mugle-wallet -p mypass info
```

### node

The wallet needs to talk to a running mugle node in order to remain up-to-date and verify its contents. By default, it tries to contact a node at 127.0.0.1:6813. To change this, either modify the value in the `mugle_wallet.toml` file, or alternatively, you can provide the `-r` (se **r** ver) switch to wallet commands.

```text
mugle-wallet -r "http://192.168.0.2:6813" info
```
